...
class Phy(Module):
    def __init__(self, eem):
        ...
        cnt = Signal(5)
        self.comb += [
            If(self.cnt_out >= mid, cnt.eq(self.cnt_out - mid),).Else(
...
class Unframer(Module):
    ...
    * `n_frame` clock cycles per frame
    ...
    def __init__(self, n_data, t_clk, n_frame):
        ...
        marker_sr = Signal(
            n_marker, reset_less=True, reset=((1 << n_marker - 1) - 1) << 1
        )
        marker_stb = Signal()
...
class Checker(Module):
    ...
    def __init__(self, n_data, t_clk, n_frame):
        ...
        self.sync += [
            ...
            If(
                self.end_of_frame | ~self.data_stb,
                self.crc.last.eq(0),
                If(
                    ~crc_good,
...
class Link(Module):
    ...
    * 1 clock lane, 6 phaser input data lanes, 1 phaser output data lane
    ...
    def __init__(self, eem):
        ...
        self.submodules.unframe = Unframer(n_data=7, n_frame=10, t_clk=8)
        self.comb += [
            self.unframe.data_in_stb.eq(self.slip.valid),
            self.unframe.data_in.eq(Cat([d[n_serde // 2 - 1] for d in self.phy.data])),
        ]
...
class Test(Module):
    def __init__(self, platform):
        ...
        if True:
            ...
            for i in range(1, 7):
                ...
                platform.add_platform_command(
                    ...
                    "[get_generated_clocks sys2] [get_ports {pin}]",
                    pin=pin,
                )
                platform.add_platform_command(
        ...
        self.sync += platform.request("user_led").eq(
            Cat(self.link.checker.frame, self.link.checker.frame_stb) == 0
        )
