...
class Iir(Module):
    def __init__(self, decoder, w_coeff, w_data, log2_a0, n_profiles, n_channels):
        ...
        self.ab = ab = Array(Array(Array(Signal((w_coeff, True)) for _ in range(n_channels))
                                   for _ in range(n_profiles)) for _ in range(NR_COEFF))
        self.offset = offset = Array(Array(Signal((w_data, True))
                                           for _ in range(n_channels)) for _ in range(n_profiles))
        # registers for selected profile for channel
        self.ch_profile = ch_profile = Array(Signal(max=n_profiles + 1)
                                             for _ in range(n_channels))
        ###
        self.xy = xy = Array(Array(Array(Signal((w_data, True)) for _ in range(n_channels))
                                   for _ in range(n_profiles)) for _ in range(NR_COEFF))
        ...
        self.ch_profile_last_ch = ch_profile_last_ch = Signal(
            max=n_profiles + 1)
        ...
        if decoder != None:
            self.comb += [ab[k][j][i].eq(decoder.get(f"ch{i}_profile{j}_coeff{k}", "write")) for i in range(
                n_channels) for j in range(n_profiles) for k in range(NR_COEFF)]
            self.comb += [offset[j][i].eq(decoder.get(f"ch{i}_profile{j}_offset", "write")) for i in range(
                n_channels) for j in range(n_profiles)]
            self.sync += [
                If(stb_out,
        ...
        self.sync += [
            ...
            If(stb_in, busy.eq(1), pp.eq(ch_profile[pc]), [[
               x0.eq(inp) for x0, inp in zip(xy[0][ch_profile[ch]], inp)] for ch in range(n_channels)]),
            If(busy,
                step.eq(step+1),
                ...
                If(step == 2,
                    ...
                    If((pc != 0) & (pc != n_channels + 1),
                        xy[2][ch_profile_last_ch][pc-1].eq(y0_clipped)))
               ),
            # if done with all channels and last data is done
            If((pc == n_channels) & (step == 2),
               pc.eq(0),
               pp.eq(ch_profile[0]),
               busy.eq(0),
               stb_out.eq(1),
               [[x1.eq(x0) for x1, x0 in zip(xy[1][ch_profile[ch]], xy[0][ch_profile[ch]])] for ch in range(n_channels)]),
        ...
        self.comb += [
            ...
            [o.eq(xy[2][ch_profile[ch]][ch])
             for o, ch in zip(outp, range(n_channels))],
            # clipping to positive output range
            y0_clipped.eq(dsp.p >> shift_c),
            If(dsp.p[-n_sign:] != 0,  # if out of output range
                y0_clipped.eq((1 << w_data - 1) - 1)),
            If(dsp.p[-1] != 0,  # if negative
                y0_clipped.eq(0)),
        ]
