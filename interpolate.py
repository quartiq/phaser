...
class InterpolateChannel(Module):
    def __init__(self):
        ...
        # scaling comments:
        # The first CIC comp filter blows up the data to 16 bits. (Gain=0.8*4)
        # Somehow the CIC input has to be scaled down a lot so it doesnt overflow.
        # I think just scaling back down once after the CIC would be better..

        # Benefit of the CIC compensator first: no overflows in HBFs
        # However CIC output downstream can still overflow
        # But this should be better (less margin required) than just HBFs

        h_fir = [24, -85, 281, -1314, 55856, -1314, 281, -85, 24]
        # ciccomp: cic droop and gain, rate 1/10, gain 2**9/5**4 ~ 0.9, 9 taps

        self.submodules.ciccomp = MAC_SYM_FIR(h_fir, 16, 16)
        ...
        # input buffer
        self.submodules.buf_in = MiniFIFO((len(self.ciccomp.input.data), True))
        ...
        scale_out = 0#len(self.hbf1.output.data) - len(self.cic.input.data) - 1
        bias_out = 0#(1 << scale_out - 1) - 1  # round half down bias
