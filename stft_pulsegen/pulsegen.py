        ...
        self.sync += [
            If(decoder.fft_stb,  # buffer addr and data if new frame in fft load mode
               ...
               datapos.eq(0),
                      ...
                      b_adr.eq(b_adr + 1),
                      If(reduce(and_, b_adr), [ff.x_in_we.eq(0) for ff in fft])#fft[0].x_in_we.eq(0))
...
class Shaper(Module):
    ...
    def __init__(self, decoder, width_d=16, size_fft=1024):
        ...
        self.sync += [
            # pulsegen settings
            inter.r.eq(decoder.get("stft_reserved", "read")),  # decoder.get("interpolation_rate", "read")),
            fft.scaling.eq(decoder.get("fft_shiftmask", "read")),
            If((decoder.get("pulse_settings", "read") & 0x01) == 0,  # continous fft outpout
               If(inter.input.ack,
                  pos.eq(pos + 1),
...
class Pulsegen(Module):
    ...
    def __init__(self, decoder, duc, width_d=16, size_fft=1024, nr_branches=3):
        ...
        self.submodules.branch = branch = [STFT_Branch(decoder, duc[i], width_d, size_fft) for i in range(nr_branches)]
