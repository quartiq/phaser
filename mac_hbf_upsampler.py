...
class MAC_HBF_Upsampler(Module):
    ...
    def __init__(self, coeff, width_d, width_coef, dsp_arch="xilinx"):
        ...
        x = [Signal((width_d, True), reset_less=True) for _ in range((len(coef) * 2))]  # input hbf
        ...
        self.p = Signal.like(p)
        self.b = Signal.like(b)
        pos_neg = Signal(len(pos)+1)

        self.comb += [
            pos_neg.eq((len(coef)*2) - 1 - pos),
            self.p.eq(p),
            self.b.eq(b),
            c.eq(bias),
            a.eq(Array(x)[pos]),
        ...
        self.sync += [
            If(~self.stop,
               ...
               pos.eq(pos+1),
               If(pos == len(coef) - 1,  # new input sample
                  pos.eq(0),
                  Cat(x).eq(Cat(self.input.data, x)),  # shift in new sample
                  self.input.ack.eq(1),
                  ),

               If(pos == dsp_pipelen - 2,
                  mux_p.eq(1),
                  ),
               ...
               If(pos == (len(coef) // 2) + dsp_pipelen - 1,  # emit trivial sample at halfway computation + pipelen
        ...
        if dsp_pipelen > (len(coef)//2):  # if dsp pipe too long
            self.sync += [
                If(pos == ((len(coef)//2) + dsp_pipelen - 1) % (len(coef)//2),
